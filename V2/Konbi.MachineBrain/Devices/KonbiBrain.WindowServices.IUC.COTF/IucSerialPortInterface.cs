...
namespace KonbiBrain.WindowServices.IUC.COTF
{
    ...
    public class IucSerialPortInterface : Object, IDisposable, IIucDeviceService
    {
        ...
        private readonly NsqMessageProducerService _nsqProducerService = new NsqMessageProducerService();
        ...
        private void ProcessResponseData(string response)
        {
            ...
            if (string.Compare(response, 0, "R923", 0, 4) == 0)
            {
                Task.Run(() =>
                {
                    ...
                    var callbackResponse = new ResponseBase() { Message = message, ResponseCode = code };
        ...
        {
            var cmd = new NsqPaymentCallbackResponseCommand(PaymentTypes.IUC_API.ToString())
            {
            ...
            LogIucApi("Check CPAS balance.");
            var response = SendAndRecv("C6180103060", "R618");
            ...
            if (responseCode == "00")
            {
                if (GetTagValue(response, 63, out string stringBalance) == 0)
                {
                    ...
                    balance = b;
                    LogIucApi($"Is CPAS card. ResponseCode {responseCode}");
                }
                else
                {
                    // Is CONTACTLESS.
                    balance = -1;
                    LogIucApi($"Is CONTACTLESS. ResponseCode {responseCode}");
                }
            }
            else
            {
        ...
        public bool EnablePayment(int cents)
        {
            if (!Port.IsOpen)
            {
                ...
                if(!ConnectPort(Port.PortName))
                    return false;
            }

           
            if (PaymentType == PaymentTypes.IUC_CEPAS)
            {
