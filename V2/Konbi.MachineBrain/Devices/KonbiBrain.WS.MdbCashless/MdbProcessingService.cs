...
namespace MdbCashlessBrain
{
    public class MdbProcessingService
    {
        ...
        public void EnableReader()
        {
            ...
            var response = new NsqEnablePaymentResponseCommand(PaymentTypes.Mdb_CASHLESS.ToString());
            response.Code = 1;
        ...
        public bool IsWaitPurchase {
            ...
            {
                isWaitPurchase = true;
                //LogService.LogMdbInfo($"Set MDB IsWaitPurchase {isWaitPurchase}");
            }
        }
        ...
        private bool SendPurchaseData(int cents, int tryingTime = 0)
        {
            ...
            //cents = 1;
            LogService.LogMdbError("send purchase cents " + cents);
            //LogService.LogMdbInfo("SendPurchase "+cents);
            ...
            //LogService.LogMdbInfo("sent purchase cents " + dataSending);
            ...
            while (true)
            {
                ...
                foreach (var afterPurchase in incomingSignalCloned)
                {
                    ...
                    if (afterPurchaseFifthByte == "4" || afterPurchaseFifthByte == "6" || afterPurchaseFifthByte == "8")//04 cancel request, 06 - Vend Denied, 08 - Cancelled
                    {
                        ...
                        LogService.LogMdbInfo("MDB Vend Denied");
                        WriteAndCheckResponse("1304");
                    ...
                    {
                        LogService.LogMdbInfo("MDB VendApproved");
        ...
        {
            LogService.LogMdbInfo("ProcessCheckMDB");
            ...
            foreach (var item in data)
            {
                switch (item.Trim())
                {
                    ...
                    case "31 30 20 30 42 0D 0A":
                        LogService.LogMdbInfo("ProcessCheckMDB::case 42:: Send reset cmd");
                        // send reset cmd
                        InitMdb();
                        break;
                }
        ...
        {
            LogService.LogMdbInfo("Disable and check response");
        ...
        private bool SendVendApproved(int tryingTime = 0)
        {
            ...
            while (true)
            {
                ...
                foreach (var sendDispatchSuccess in incomingSignalCloned)
                {
                    ...
                    {
                        LogService.LogMdbInfo("MDB VendSuccess");
        ...
        private List<string> WriteAndCheckResponse(string hexString, int tryingTime = 0)
        {
            ...
            LogService.LogMdbInfo($"MDB: WriteAndCheckResponse - {hexString}");
            //Cancel reader first
            ResetIncomingData();
            var beginTime = DateTime.Now;
            WriteData(hexString);
        ...
        public bool WriteData(string message)
        {
            if (port != null)
            {
                if (port.IsOpen == true)
                {
                    //LogService.LogMdbInfo("Write data: " + message);
        ...
        {
            LogService.LogMdbInfo("ProceedCardSwipe");
        ...
        {
            LogService.LogMdbInfo($"MDB incoming: {hexData}");
