...
namespace MdbCashlessBrain
{
    public class MdbProcessingService
    {
        ...
        public void EnableReader()
        {
            ...
            var command = new MdbCashlessResponseCommand();
            var commandObj = new
            {
                TranCode = "1234",
                CustomerMessage = "Enabling payment device, please tap your card.",
                State = PaymentState.ActivatedPaymentSuccess,
                ResponseObject = "",
                OtherInfo = ""
            };
            command.CommandObject = commandObj;
        ...
        public bool SendPurchase(double amount)
        {
            if (amount == 0) return false;
            EventAggregator.BeginPublishOnUIThread($"SendPurchase {amount}");
        ...
        private bool SendPurchaseData(int cents, int tryingTime = 0)
        {
            ...
            EventAggregator.BeginPublishOnUIThread("sent purchase cents " + dataSending);
            while (true)
            {
                ...
                foreach (var afterPurchase in incomingSignalCloned)
                {
                    ...
                    if (afterPurchaseFifthByte == "4" || afterPurchaseFifthByte == "6" || afterPurchaseFifthByte == "8")//04 cancel request, 06 - Vend Denied, 08 - Cancelled
                    {
                        ...
                        EventAggregator.BeginPublishOnUIThread("Vend Denied");
        ...
        private bool SendVendApproved(int tryingTime = 0)
        {
            ...
            EventAggregator.BeginPublishOnUIThread("Vend Approved");
            var beginTime = DateTime.Now;
            WriteAndCheckResponse("13020001");//SendDispatchSuccess
            EventAggregator.BeginPublishOnUIThread("vend success");
            Thread.Sleep(500);
            WriteAndCheckResponse("1304");
        ...
        {
            EventAggregator.BeginPublishOnUIThread("WriteAndCheckResponse " + hexString);
        ...
        public bool WriteData(string message)
        {
            if (port != null)
            {
                if (port.IsOpen == true)
                {
                    ...
                    EventAggregator.Publish("Write data: " + message, a => { Task.Factory.StartNew(a).LogTaskExceptions(); });
                    port.DtrEnable = true;
        ...
        private void ProcessIncomingData(string hexData)
        {
            try
            {
                EventAggregator.BeginPublishOnUIThread(hexData);
        ...
        private void SendResultToMain(MdbCashlessResponseResult cashlessResponseResult)
        {
            ...
            {
                TranCode = "1234",
                CustomerMessage = state.Item2,
                State =state.Item1,
                ResponseObject = "",
                OtherInfo = $"MdbCashlessResponseResult: {cashlessResponseResult}"
            };
            command.CommandObject = commandObj;
            NsqMessageProducerService.SendNsqCommand(NsqTopics.PAYMENT_RESPONSE_TOPIC, command);
        ...
        private void AddIncomingData(string hexData)
        {
            ...
            LockAndAccessIncomingData(() => {
                ...
                if (count > 0 && !incommingSignals[count - 1].Trim().EndsWith(MDB_RESPONSE_END))
                {
                    ...
                    EventAggregator.BeginPublishOnUIThread("Modify last: "+last);
                }
