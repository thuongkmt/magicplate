...
import {
    CategoryServiceProxy, SessionsServiceProxy, CategoryDto,
    SessionDto, ProductServiceProxy,
    ProductDto, ProductMenuDto, ProductMenusServiceProxy, PlateMenusInput,
    AssignPlateModelInput, Plate, ImportResult, ImportData, ReplicateInput,
    PlateCategoriesServiceProxy, PlateCategoryDto,
    PlatesServiceProxy, GetPlateForView
} from '@shared/service-proxies/service-proxies';
import { } from '@shared/service-proxies/plate-menu-service-proxies';
import { } from '@shared/service-proxies/plate-category-service-proxies';
...
export class PlateMenusComponent extends AppComponentBase {
    ...
    constructor(
        ...
        private _plateCategoriesServiceProxy: PlateCategoriesServiceProxy,
    ...
    getSessions() {
        ...
        this._sessionsServiceProxy.getAll(
            ...
            if (result.items != null) {
                ...
                this.callProductMenusApi(this.lazyLoadEvent);
            }
        });
    }
    ...
    getProductMenus(event?: LazyLoadEvent) {
        ...
        this.callProductMenusApi(event);
    }

    callProductMenusApi(event?: LazyLoadEvent) {
        this.primengTableHelper.showLoadingIndicator();
    ...
    // updateProductForPlateMenu(event: any, record: ProductMenuDto) {
    //     if (record === null || record === undefined) {
    //         return;
    //     }

    //     let productId = event.target.value;
    //     let plateMenuId = record.id;

    //     let input = new ProductMenuInputDto();
    //     input.id = plateMenuId;
    //     input.productId = productId;

    //     this._plateMenusServiceProxy.updateProduct(input)
    //         .subscribe(result => {
    //             if (result === true) {
        ...
        myReader.onloadend = (e) => {
            ...
            this.papa.parse(fileContent, {
                ...
                complete: (result) => {
                    ...
                    //submit data
                    this._plateMenusServiceProxy.importProductMenu(importData)
                        .subscribe((result) => {
    ...
    syncData() {
        // Sync sessions.
        this.primengTableHelper.showLoadingIndicator();
        // this._sessionsServiceProxy.syncSession()
        // .finally(() => { })
        // .subscribe(() => {
        this._plateMenusServiceProxy.syncProductMenuData()
            .finally(() => {
                this.primengTableHelper.hideLoadingIndicator();
            })
            .subscribe(() => {
