...
export class PlateServiceProxy {
     ...
     * @return Success
     */
    getPlateForView(id: string): Observable<GetPlateForEditOutput> {
    ...
    getPlateInventory(): Observable<PagedResultDtoOfGetPlateInventory> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_: any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<PagedResultDtoOfGetPlateInventory>><any>_observableThrow(e);
                }
            } else
                return <Observable<PagedResultDtoOfGetPlateInventory>><any>_observableThrow(response_);
        }));
    }

    protected processGetAllDish(response: HttpResponseBase): Observable<PagedResultDtoOfGetPlateInventory> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
                (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
                let result200: any = null;
                let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
                result200 = resultData200 ? PagedResultDtoOfGetPlateInventory.fromJS(resultData200) : new PagedResultDtoOfGetPlateInventory();
        ...
        return _observableOf<PagedResultDtoOfGetPlateInventory>(<any>null);
    }
...
export class PlateInventoryDto {
    ...
    static fromJS(data: any): PlateInventoryDto {
        data = typeof data === 'object' ? data : {};
        let result = new PlateInventoryDto();
        result.init(data);
        return result;
    }
...
export class PagedResultDtoOfGetPlateInventory {
    totalCount!: number | undefined;
    items!: PlateInventoryDto[] | undefined;

    constructor(data?: PagedResultDtoOfGetPlateInventory) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(data?: any) {
        if (data) {
            ...
            if (data["items"] && data["items"].constructor === Array) {
                this.items = [];
                for (let item of data["items"])
                    this.items.push(PlateInventoryDto.fromJS(item));
            }
        }
    }

    static fromJS(data: any): PagedResultDtoOfGetPlateInventory {
        data = typeof data === 'object' ? data : {};
        let result = new PagedResultDtoOfGetPlateInventory();
        result.init(data);
        return result;
    }
