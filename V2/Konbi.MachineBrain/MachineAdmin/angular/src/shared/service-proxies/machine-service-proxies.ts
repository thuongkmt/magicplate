...
export class MachineServiceProxy {
    ...
    */
    ...
    getMachineFromServer(id: string): Observable<MachineSyncDto> {
        ...
        return this.http.request("get", url_, options_).flatMap((response_: any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<MachineSyncDto>><any>Observable.throw(e);
                }
            } else
                return <Observable<MachineSyncDto>><any>Observable.throw(response_);
        });
    }

    protected processGetMachineFromServer(response: HttpResponseBase): Observable<MachineSyncDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
                (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            return blobToText(responseBlob).flatMap(_responseText => {
                let result200: any = null;
                let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
                result200 = resultData200 ? MachineSyncDto.fromJS(resultData200) : new MachineSyncDto();
                return Observable.of(result200);
            });
        } else if (status === 401) {
            return blobToText(responseBlob).flatMap(_responseText => {
        ...
        return Observable.of<MachineSyncDto>(<any>null);
    }

    updateMachineToServer(input: MachineSyncDto | null | undefined): Observable<MachineSyncDto> {
    ...
    protected processUpdateMachineToServer(response: HttpResponseBase): Observable<MachineSyncDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
                (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            return blobToText(responseBlob).flatMap(_responseText => {
                let result200: any = null;
                let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
                result200 = resultData200 ? resultData200 : new MachineSyncDto();
                return Observable.of(result200);
            });
        } else if (status === 401) {
            return blobToText(responseBlob).flatMap(_responseText => {
...
export class MachineSyncDto {
    id: string | undefined;
    name: string;

    constructor(data?: MachineSyncDto) {
        if (data) {
    ...
    static fromJS(data: any): MachineSyncDto {
        data = typeof data === 'object' ? data : {};
        let result = new MachineSyncDto();
        result.init(data);
        return result;
    }
