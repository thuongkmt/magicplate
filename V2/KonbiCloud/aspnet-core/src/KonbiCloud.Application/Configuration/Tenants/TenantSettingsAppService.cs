...
namespace KonbiCloud.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        public async Task<TenantSettingsEditDto> GetAllSettings()
        {
            ...
            settings.Magicplate = await GetMagicplateSettingsAsync();
        ...
        private async Task<MagicplateSettingsEditDto> GetMagicplateSettingsAsync()
        {
            ...
            return new MagicplateSettingsEditDto()
            {
                TaxSettings = new TaxSettingsEditDto()
                {
        ...
        public async Task UpdateAllSettings(TenantSettingsEditDto input)
        {
            ...
            await UpdateMagicplateSettingsAsync(input.Magicplate);
        ...
        private async Task UpdateMagicplateSettingsAsync(MagicplateSettingsEditDto input)
        {
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.MagicplateSettings.TaxSettings.TaxName, input.TaxSettings.Name);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.MagicplateSettings.TaxSettings.TaxType, input.TaxSettings.Type);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(), AppSettings.MagicplateSettings.TaxSettings.TaxPercentage, input.TaxSettings.Percentage.ToString());

            await _sendMessageToMachineService.SendNotificationAsync(new KeyValueMessage() { Key = MessageKeys.Settings });
        }
