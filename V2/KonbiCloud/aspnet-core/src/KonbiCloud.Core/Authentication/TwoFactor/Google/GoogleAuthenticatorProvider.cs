...
namespace KonbiCloud.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : KonbiCloudServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("KonbiCloud", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
