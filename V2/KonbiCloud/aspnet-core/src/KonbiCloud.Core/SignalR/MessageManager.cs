...
{
    [AbpAuthorize]
    public class MessageManager : KonbiCloudDomainServiceBase, IMessageManager
    {
        private readonly IFriendshipManager _friendshipManager;
        private readonly IMessageCommunicator _chatCommunicator;
        private readonly IOnlineClientManager<MessageChannel> _onlineClientManager;
        private readonly UserManager _userManager;
        private readonly ITenantCache _tenantCache;
        private readonly IUserFriendsCache _userFriendsCache;
        private readonly IUserEmailer _userEmailer;
        private readonly IRepository<GeneralMessage, long> _chatMessageRepository;
        private readonly IMessageFeatureChecker _chatFeatureChecker;

        public MessageManager(
            IFriendshipManager friendshipManager,
            IMessageCommunicator chatCommunicator,
            IOnlineClientManager<MessageChannel> onlineClientManager,
            UserManager userManager,
            ITenantCache tenantCache,
            IUserFriendsCache userFriendsCache,
            IUserEmailer userEmailer,
            IRepository<GeneralMessage, long> chatMessageRepository,
            IMessageFeatureChecker chatFeatureChecker)
        {
        ...
        public async Task SendMessageAsync(UserIdentifier sender, UserIdentifier receiver, string message, string senderTenancyName, string senderUserName, Guid? senderProfilePictureId)
        {
            ...
            _chatFeatureChecker.CheckMessageFeatures(sender.TenantId, receiver.TenantId);
        ...
        [UnitOfWork]
        public virtual long Save(GeneralMessage message)
        {
        ...
        public virtual int GetUnreadMessageCount(UserIdentifier sender, UserIdentifier receiver)
        {
            using (CurrentUnitOfWork.SetTenantId(receiver.TenantId))
            {
                return _chatMessageRepository.Count(cm => cm.UserId == receiver.UserId &&
                                                          ...
                                                          cm.ReadState == MessageReadState.Unread);
            }
        }

        public async Task<GeneralMessage> FindMessageAsync(int id, long userId)
        {
        ...
        private async Task HandleSenderToReceiverAsync(UserIdentifier senderIdentifier, UserIdentifier receiverIdentifier, string message, Guid sharedMessageId)
        {
            ...
            var sentMessage = new GeneralMessage(
                senderIdentifier,
                receiverIdentifier,
                MessageSide.Sender,
                message,
