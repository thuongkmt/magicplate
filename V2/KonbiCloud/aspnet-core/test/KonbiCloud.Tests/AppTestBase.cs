...
using KonbiCloud.Tests.TestDatas;
...
namespace KonbiCloud.Tests
{
    ...
    /// Provides methods to easily work with <see cref="KonbiCloudDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<KonbiCloudTestModule>
    {
        ...
        {
            void NormalizeDbContext(KonbiCloudDbContext context)
            {
        ...
        protected void UsingDbContext(Action<KonbiCloudDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<KonbiCloudDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<KonbiCloudDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<KonbiCloudDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<KonbiCloudDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KonbiCloudDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<KonbiCloudDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KonbiCloudDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<KonbiCloudDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<KonbiCloudDbContext, Task<T>> func)
        {
            T result;
